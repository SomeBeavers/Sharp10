...
namespace InterpolatedStringImprovements_net6.Playground1
{
    ...
    public ref struct DefaultInterpolatedStringHandler
    {
            ...
            if (value.Length == 2)
            {
                ...
                if ((uint)pos < chars.Length - 1)
                {
                    ...
                    Unsafe.WriteUnaligned(
                        ...
                        Unsafe.ReadUnaligned<int>(ref Unsafe.As<char, byte>(ref source)));
                    this._pos = pos + 2;
                }
                else
                {
        ...
        public void AppendFormatted<T>(T value)
        {
            ...
            if (value is IFormattable)
            {
                ...
                {
                    int charsWritten;
                    while (!((ISpanFormattable)value).TryFormat(this._chars.Slice(this._pos), out charsWritten, default, this._provider)) // constrained call avoiding boxing for value types
                    {
                        Grow();
                    }
                ...
                s = ((IFormattable)value).ToString(format: null, this._provider); // constrained call avoiding boxing for value types
            }
            else
            {
                s = value?.ToString();
        ...
        public void AppendFormatted<T>(T value, string? format)
        {
            ...
            if (value is IFormattable)
            {
                ...
                {
                    int charsWritten;
                    while (!((ISpanFormattable)value).TryFormat(this._chars.Slice(this._pos), out charsWritten, format, this._provider)) // constrained call avoiding boxing for value types
                    {
                        Grow();
                    }
                ...
                s = ((IFormattable)value).ToString(format, this._provider); // constrained call avoiding boxing for value types
            }
            else
            {
                s = value?.ToString();
        ...
        public void AppendFormatted(string? value)
        {
            ...
            if (!this._hasCustomFormatter &&
                value is not null &&
                value.TryCopyTo(this._chars.Slice(this._pos)))
            {
                this._pos += value.Length;
            }
            else
        ...
        private void Grow(int additionalChars)
        {
            ...
            GrowCore((uint)this._pos + (uint)additionalChars);
        }
        ...
        private void Grow()
        {
            ...
            GrowCore((uint)this._chars.Length + 1);
        }
        ...
        private void GrowCore(uint requiredMinCapacity)
        {
            ...
            uint newCapacity = Math.Max(requiredMinCapacity, Math.Min((uint)this._chars.Length * 2, 1));
